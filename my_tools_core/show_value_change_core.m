function r=show_value_change_core(options)
    ...
    if isempty(options.data)
        if string(options.varname)=="uv"
            ...
            p=options.position; u1=read_data(options.file,'u',p,options.time_range);
            p(2)=p(2)+1; u2=read_data(options.file,'u',p,options.time_range);
            u=0.5*(u1+u2);
            p=options.position; v1=read_data(options.file,'v',p,options.time_range);
            p(1)=p(1)+1; v2=read_data(options.file,'v',p,options.time_range);
            v=0.5*(v1+v2);
            data=u.^2+v.^2;
            clear u1 u2 v1 v2 u v
            data=sqrt(data);
        else
            data=read_data(options.file,options.varname,options.position,options.time_range);
        end
    else
        data=options.data;
    end
...
function r=read_data(file,var,position,time_range)
    try
        nc=netcdf.open(file);
